...
enchant.util.MutableText = enchant.Class.create(enchant.Sprite, {
	...
	redraw: function() {
		var i, x, y, wNum, charCode, charPos, baseX = 0, baseY = 0;
		if (this._centered) {
			baseX = (this.width - this._text.length * this.fontSize) / 2;
			baseY = (this.height - this.fontSize) / 2;
		}
		...
		for(i=0; i<this._text.length; i++) {
			...
			this.image.draw(enchant.Game.instance.assets['font.png'], 
				...
				(i%this.returnLength)*this.fontSize+baseX, ((i/this.returnLength)|0)*this.fontSize+baseY, this.fontSize, this.fontSize);
		}
	},
	centered: {
		get: function() {
...
enchant.util.TimeLabel = enchant.Class.create(enchant.util.MutableText, {
	initialize: function(x, y, counttype) {
		...
		this.addEventListener('enterframe', function(){
			...
			this.text = this.label + (this._time / enchant.Game.instance.fps).toFixed(0);
		});
	},
	time: {
		get: function() {
